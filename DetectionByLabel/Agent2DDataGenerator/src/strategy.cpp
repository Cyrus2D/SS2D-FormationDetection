...
Strategy::updatePosition( const WorldModel & wm )
{
    ...
    //getting data for formation prediction
    static int hamin=85;
    if(hamin==wm.time().cycle())
        for(ball_pos.x=-52;ball_pos.x < 53;ball_pos.x++)
            for(ball_pos.y=-34;ball_pos.y < 35;ball_pos.y++) {
                std::cout<<"&& "<<ball_pos.x<<" "<<ball_pos.y<<" ";
                f->getPositions( ball_pos, M_positions );
                int unum;
                for(unum = 1;unum < 12;unum++)
                    std::cout<<M_positions[unum-1].x<<" "<<M_positions[unum-1].y<<" ";
                std::cout<<"631";
                if(ball_pos.x == 52 && ball_pos.y == 34)
                    break;
                else
                    std::cout<<std::endl;
                M_positions.clear();
